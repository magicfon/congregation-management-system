...
(async()=>{
 ...
 assert.equal(heat.regionHeat([90],[{...areas[0],isDispatched:true,areaId:'a90'}],today,true).isDispatched,true);
 ...
 const route=load('src/app/api/area-status/[mapId]/route.ts',{
  ...
  '../../../../lib/allocation':allocation,'../../../../lib/boundary-drafts':boundary,'../../../../lib/area-status':heat,'../../../../lib/google-sheets':dates,'../../../../lib/report-completion-sync':{COMPLETION_SYNC_KEY:'test'},
  '../../../../lib/db':{prisma:{$queryRaw:async(strings,...values)=>{assert(strings.join('').startsWith('SELECT'));assert(['nanzih','chiaotou','tzuguan'].includes(values[0]));queryCount++;if(fail)throw Error('offline');return draft?[draft]:[]},area:{findMany:async query=>{assert.deepEqual(JSON.parse(JSON.stringify(query.select)),{id:true,assignedMemberId:true,dispatchedAt:true,completedAt:true,mapId:true,sheetNo:true,name:true,lastReportedCompletedAt:true});assert.deepEqual(Array.from(query.where.mapId.in),['nanzih','chiaotou','tzuguan']);return [{mapId:'nanzih',sheetNo:58,name:'58',lastReportedCompletedAt:new Date('2026-01-01T00:00:00Z')},{mapId:'nanzih',sheetNo:999,name:'unlocated',lastReportedCompletedAt:null},{mapId:'tzuguan',sheetNo:150,name:'150',lastReportedCompletedAt:new Date('2025-01-01T00:00:00Z')}]}},setting:{findUnique:async()=>({value:'2026-09-26T00:00:00Z'})}}},
 });
