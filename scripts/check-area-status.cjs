...
(async()=>{
 ...
 assert.equal(heat.heatColor(0),'#22c55e');assert.equal(heat.heatColor(90),'#facc15');assert.equal(heat.heatColor(180),'#ef4444');assert.equal(heat.heatColor(999),'#ef4444');assert.equal(heat.heatColor(null),'#64748b');assert.notEqual(heat.heatColor(45),heat.heatColor(0));assert.notEqual(heat.heatColor(135),heat.heatColor(90));
 ...
 const route=load('src/app/api/area-status/[mapId]/route.ts',{
  ...
  '../../../../lib/db':{prisma:{$queryRaw:async(strings,...values)=>{assert(strings.join('').startsWith('SELECT'));assert.equal(values[0],'nanzih');queryCount++;if(fail)throw Error('offline');return draft?[draft]:[]},area:{findMany:async query=>{assert.deepEqual(JSON.parse(JSON.stringify(query.select)),{sheetNo:true,name:true,lastReportedCompletedAt:true});assert.equal(query.where.mapId,'nanzih');return [{sheetNo:58,name:'58',lastReportedCompletedAt:new Date('2026-01-01T00:00:00Z')},{sheetNo:999,name:'unlocated',lastReportedCompletedAt:null}]}},setting:{findUnique:async()=>({value:'2026-09-26T00:00:00Z'})}}},
 });
 ...
 const originalResponse=await (await route.GET({},context)).json();assert.equal(originalResponse.boundary.source,'original');assert(originalResponse.unlocatedNumbers.includes(999));assert(originalResponse.regions.some(r=>r.numbers.includes(58)&&r.days>=180));
 ...
 const cloud=await (await route.GET({},context)).json();assert.equal(cloud.boundary.source,'cloud');assert.equal(cloud.boundary.version,7);assert.equal(cloud.regions.length,saved.candidates.length);
