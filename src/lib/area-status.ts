...
export type CompletionArea = { sheetNo: number | null; name: string; lastCompletedDate: string | null; areaId?: string; isDispatched?: boolean }
export type HeatReport = { number: number; name: string | null; lastCompletedDate: string | null; days: number | null; areaId: string | null; isDispatched: boolean }
...
export function regionHeat(numbers: number[], areas: CompletionArea[], today: string, synced: boolean, maxDays: number | null = maximumReportDays(areas, today)) {
  const reports: HeatReport[] = [...new Set(numbers)].sort((a, b) => a - b).map(number => {
    ...
    return { number, areaId: area?.areaId || null, isDispatched: area?.isDispatched === true, name: area?.name || null, lastCompletedDate: area?.lastCompletedDate || null, days: idleCalendarDays(area?.lastCompletedDate || null, today) }
  })
  ...
  return { status, days, isDispatched: reports.some(r => r.isDispatched), color: heatColor(days, maxDays), reports }
}
